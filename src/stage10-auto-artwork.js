...
async function run(panel){
  ...
  setState(panel,'loading','Analyze → prompt adapt → transform → compose → QA');
  try{
    ...
    panel.querySelector('[data-art-adapter]').textContent=result.prompt?.themeAdapter||'-';
    panel.querySelector('[data-art-prompt]').textContent=result.prompt?.version||MASTER_TRANSFORM_PROMPT_VERSION;
    panel.querySelector('[data-art-note]').textContent=result.transformed.fallback?'Master Transform Prompt V1 + theme adapter sudah dirakit otomatis. Provider generatif belum terhubung, jadi preview memakai source-preserving fallback. Saat provider aktif, payload.prompt.positive + payload.prompt.negative langsung siap dipakai.':'Generative provider aktif; Master Transform Prompt V1 + automatic theme adapter dipakai untuk artwork transform.';
    ...
    window.weddingAutoArtworkLastRun=result;window.weddingAutoArtworkPrompt=result.prompt;
...
function boot(){
  ...
  panel.innerHTML=`<div class="auto-art-head"><div><span>Stage #10.9</span><strong>Auto Artwork Transform Engine</strong></div><i></i></div><label class="auto-art-file"><input id="auto-artwork-file" type="file" accept="image/*"><span data-art-file>Choose source image</span></label><label class="auto-art-label">Artwork preset<select id="auto-artwork-preset"></select></label><div class="auto-art-grid"><div><span>Provider</span><strong data-art-provider>-</strong></div><div><span>Analysis</span><strong data-art-analysis>not run</strong></div><div><span>Composition</span><strong data-art-layout>-</strong></div><div><span>Theme Adapter</span><strong data-art-adapter>automatic</strong></div><div><span>Prompt</span><strong data-art-prompt>${MASTER_TRANSFORM_PROMPT_VERSION}</strong></div><div><span>Layers</span><strong data-art-layers>-</strong></div><div><span>Quality</span><strong data-art-qa>-</strong></div><div><span>Version</span><strong>${ARTWORK_ENGINE_VERSION}</strong></div></div><button type="button" data-art-run class="auto-art-run">AUTO CREATE ARTWORK</button><div class="auto-art-status" data-art-status>Ready for source image</div><p data-art-note>Master Transform Prompt V1 aktif. Preset otomatis memilih theme adapter, lalu Scene Analyzer menyisipkan horizon, subject center, negative space, safe text area, dan structure locks ke prompt provider.</p>`;
  ...
  window.weddingAutoArtwork={engine,run:options=>engine.run(options),version:ARTWORK_ENGINE_VERSION,promptVersion:MASTER_TRANSFORM_PROMPT_VERSION};
  const header=document.querySelector('.brand-wrap span');if(header) header.textContent='Master Transform Prompt V1 · Stage #10.9';
  const checkpoint=document.querySelector('.checkpoint');if(checkpoint) checkpoint.textContent='#10.9 Master Transform Prompt V1 · Auto Theme Adapter + Scene Adapter';
}

window.addEventListener('pagehide',revoke,{once:true});
boot();
console.info(`[Wedding Template Studio] Auto Artwork Transform Engine ${ARTWORK_ENGINE_VERSION} · ${MASTER_TRANSFORM_PROMPT_VERSION} booting.`);
