...
function normalizeGeneratedProject(result){
  ...
  cover.fidelity={...(cover.fidelity||{}),system:'auto-artwork-transform',handoff:'generated-fullbleed-10.10F'};
  cover.renderHandoff='10.10F';
  cover.layers=[
    {
    ...
    {id:'cover-title',kind:'text',role:'content',content:'THE WEDDING OF',transform:{x:50,y:17,width:'76%',opacity:1},motion:{preset:'fade-in',durationMs:700}},
    {id:'cover-names',kind:'text',role:'content',content:names,transform:{x:50,y:25,width:'86%',opacity:1},motion:{preset:'fade-up',durationMs:800}},
    {id:'cover-guest-label',kind:'text',role:'content',content:'Kepada Bapak/Ibu/Saudara/i',transform:{x:50,y:40,width:'78%',opacity:1},motion:{preset:'fade-in',durationMs:650}},
    {id:'cover-guest',kind:'text',role:'content',content:guest,transform:{x:50,y:44,width:'78%',opacity:1},motion:{preset:'fade-up',durationMs:700}},
    {id:'cover-location',kind:'text',role:'content',content:'Di Tempat',transform:{x:50,y:48,width:'62%',opacity:1},motion:{preset:'fade-in',durationMs:650}},
    {id:'cover-open',kind:'button',role:'button',content:'Buka Undangan',transform:{x:50,y:57,width:'150px',opacity:1},motion:{preset:'fade-up',durationMs:750}}
  ];
  cover.timeline=[];
  project.generator={...(project.generator||{}),mode:'auto-artwork-transform',handoff:'10.10F',generatedArtworkIsolated:true};
  project.fidelity={...(project.fidelity||{}),mode:'auto-artwork-transform',handoff:'10.10F'};
  return project;
}
...
function markGeneratedCover(project){
  requestAnimationFrame(()=>requestAnimationFrame(()=>{
    ...
    if(!section)return;
    section.classList.add('auto-generated-cover-handoff');
    section.dataset.autoArtworkHandoff='10.10F';
  }));
}
...
async function run(panel){
  ...
  setText(panel,'[data-art-output]','WAITING FOR AI');setText(panel,'[data-art-fallback]','-');setText(panel,'[data-art-handoff]','WAITING');
  setState(panel,'loading','Analyze → Master Prompt V1 → Workers AI → trace → clean handoff → Pixi');
  try{
    ...
    setText(panel,'[data-art-handoff]',result.transformed.fallback?'FALLBACK':'CLEAN · 10.10F');
    ...
    panel.querySelector('[data-art-note]').textContent=result.transformed.fallback
      ...
      :trace.state==='SOURCE REUSED'
        ?'Provider mengembalikan source asli; clean handoff tidak bisa memperbaiki output provider.'
        :`Stage #10.10F aktif: AI composite diisolasi sebagai full-bleed base, legacy Joglo/arch/floral dibuang dari cover generated, lalu typography + CTA dibangun ulang sebelum Pixi mount.`;
    setState(panel,q.ok?'ready':'warning',trace.state==='SOURCE REUSED'?'Provider output reused source':q.ok?'AI artwork mounted · clean handoff':'Artwork candidate needs QA');
  }catch(error){
    console.error('[Auto Artwork Transform]',error);const message=error?.message||'Artwork transform failed';
    setState(panel,'error',message);setText(panel,'[data-art-output]','ERROR');setText(panel,'[data-art-handoff]','ERROR');
...
function boot(){
  ...
  const header=document.querySelector('.brand-wrap span');if(header) header.textContent=`Cloudflare Workers AI Render Handoff · Stage #${STAGE_REVISION}`;
  const checkpoint=document.querySelector('.checkpoint');if(checkpoint) checkpoint.textContent=`#${STAGE_REVISION} AI Composite → Clean Cover → Typography → Pixi`;
}

window.addEventListener('pagehide',revoke,{once:true});
boot();
console.info(`[Wedding Template Studio] Stage #${STAGE_REVISION} generated artwork render handoff active.`);
