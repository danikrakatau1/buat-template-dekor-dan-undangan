...
function isFidelity(scene,project){ return Boolean(scene?.fidelity?.system==='hd-vector-layered' || project?.fidelity?.mode==='hd-vector-layered' || scene?.fidelity?.system==='auto-artwork-transform' || project?.generator?.mode==='auto-artwork-transform' || project?.generator?.referenceNativeRebuild); }
...
async function mountHybrid(project=currentProject()){
  ...
  const isAuto=scene?.fidelity?.system==='auto-artwork-transform'||project?.generator?.referenceNativeRebuild,handoff=autoHandoff(scene,project);
  const disableEditorOverlay=Boolean(project?.generator?.disableEditorOverlay || scene?.disableEditorOverlay || project?.generator?.referenceNativeRebuild || /^10\.10[A-Z]/.test(handoff));
  ...
  try{
    ...
    auxiliary=await mountAuxiliaryLayers(scene,section);const audit=updateRegistryUI();section.dataset.hybridFidelity='ready';section.dataset.artworkOptimization=project?.generator?.referenceNativeRebuild?'stage11-reference-native':isAuto?(handoff?`auto-artwork-${handoff}`:'auto-artwork-runtime'):'9.9.5';section.dataset.assetRegistry=FINAL_ASSET_REGISTRY_VERSION;section.dataset.productionPromotionPending=String(audit?.productionPromotionPending?.length || 0);
...
import('./stage11-reference-rebuild.js').catch(error=>console.error('[Stage 11 Reference Rebuild]',error));
window.addEventListener('beforeunload',cleanup,{once:true});
console.info('[Wedding Template Studio] Hybrid renderer + Auto Artwork + Stage 11 reference rebuild bootstrap active.');
