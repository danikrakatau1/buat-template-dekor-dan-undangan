...
function layer(id, generator, role, transform={}, motion={}) {
  return {
    ...
    kind:'decor',
    role,
    asset:{ type:'procedural', generator, seed:motion.seed || id, variant:motion.variant || 'soft', count:motion.count },
    transform,
    motion:{
  ...
  return [
    {
      ...
      motion:{ preset:variation.motion.title, delayMs:1450, durationMs:850, easing:'premium', once:true }
    },
    {
      id:'couple-name', kind:'text', role:'content', content:names,
      motion:{ preset:variation.motion.names, delayMs:1900, durationMs:1000, easing:'premium', once:true }
    },
    {
...
function baseCover({ preset, background, atmosphere, decor, variation, title, names }) {
  return {
    ...
    variation:{
      ...
      heroVariant:variation.heroVariant,
      backgroundMotion:variation.backgroundMotion
    },
    background:{ ...background, kenBurns:variation.backgroundMotion },
    atmosphere:{ ...atmosphere, effects:variation.atmosphere },
    timeline:commonTimeline,
    layers:[...decor, ...textLayers(variation, title, names)],
    responsive:{
...
function jawaCover(variation) {
  const x = variation.decorX;
  const y = variation.decorY;
  ...
  return baseCover({
    preset:'jawa-cinematic', variation,
    background:{ type:'procedural', transition:'cinematic-crossfade', durationMs:6000, transitionMs:1200 },
    atmosphere:{ intensity:.62 },
    decor:[
...
function oceanCover(variation) {
  ...
  return baseCover({
    preset:'ocean-dream', variation,
    background:{ type:'procedural', transition:'crossfade', durationMs:7000, transitionMs:1500 },
    atmosphere:{ intensity:.55 },
    decor:[
...
function celestialCover(variation) {
  const frontGenerator = variation.frontDecor;
  const backGenerator = variation.backDecor;
  return baseCover({
    preset:'celestial-dream', variation,
    background:{ type:'procedural', transition:'cosmic-dissolve', durationMs:8000, transitionMs:1800 },
    atmosphere:{ intensity:.58 },
    decor:[
...
export function createThemeProject(themeId='jawa-luxury', seed) {
  ...
  return {
    project:{ id:`demo-${pack.id}`, name:`${pack.label} Demo`, version:1, preset:pack.id, seed:resolvedSeed },
    theme:{
      id:pack.id,
      palette:pack.palette,
      tokens:{ ...pack.tokens, motionIntensity:variation.motionIntensity, decorIntensity:variation.decorIntensity }
    },
    variation,
    audio:{ src:'', autoplayAfterOpen:true, loop:true, volume:.75, fadeInMs:1200 },
    scenes:[pack.createCover(variation), ...emptyScenes.map(scene => ({...scene}))]
  };
}
...
export function getThemeSeedPrefix(themeId){
  return (THEME_PACKS[themeId] || THEME_PACKS['jawa-luxury']).seedPrefix;
}
